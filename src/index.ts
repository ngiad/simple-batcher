...
//     constructor(batchFunction: (keys: K[]) => Promise<(T | Error | null)[]>, delay: number = 0) {
...
//   constructor(memoryLimitMB: number = 1024, expirationTime: number = 0) {
//     super();
//     this.memoryLimitMB = memoryLimitMB;
//     this.expirationTime = expirationTime;
//   }

//   async init(initStore?: Record<string, T>): Promise<void> {
//     this.memoryStore = initStore || {};
//   }
...
//   async insertMany(keyValuePairs: Record<string, T>): Promise<any[]> {
//     if (!(await this.checkMemoryAvailability(keyValuePairs))) {
//       throw new Error("Not enough memory to save data.");
//     }
...
//     this.emit('insertMany', { keyValuePairs });
//     return results;
//   }

//   async updateValue(key: string, value: T): Promise<Record<string, T>> {
//     if (!this.memoryStore.hasOwnProperty(key)) {
//       throw new Error("Key does not exist. Use setValue to create a new key-value pair.");
//     }
//     if (await this.checkMemoryAvailability({ [key]: value })) {
//       return this.setValue(key, value);
//     }
//     throw new Error("Not enough memory to update data.");
//   }
...
//   async exportValue(key: string): Promise<{ value: T; expiryTime?: number }> {
//     if (!this.memoryStore.hasOwnProperty(key)) {
//       throw new Error(`Key "${key}" does not exist.`);
//     }
//     const value = this.memoryStore[key];
//     const expiryTime = this.expirationTime > 0 ? this.expirationTime : undefined;
//     this.emit('exportValue', { key, value, expiryTime });
//     return { value, ...(expiryTime !== undefined ? { expiryTime } : {}) };
//   }

//   async exportAll(): Promise<Record<string, { value: T; expiryTime?: number }>> {
//     const allValues: Record<string, { value: T; expiryTime?: number }> = {};
//     for (const key in this.memoryStore) {
//       const value = this.memoryStore[key];
//       const expiryTime = this.expirationTime > 0 ? this.expirationTime : undefined;
//       allValues[key] = { value, ...(expiryTime !== undefined ? { expiryTime } : {}) };
//     }
//     this.emit('exportAll', allValues);
//     return allValues;
//   }
...
//   async getMultiple(keys: string[]): Promise<Record<string, T | undefined>> {
//     const values: Record<string, T | undefined> = {};
//     for (const key of keys) {
//       values[key] = this.memoryStore[key];
//     }
//     this.emit('getMultiple', { keys, values });
//     return values;
//   }

//   async store(): Promise<Record<string, T>> {
//     this.emit('store', this.memoryStore);
//     return this.memoryStore;
//   }
// }
...
//     this.queue = [];
//     this.scheduled = false;
//     this.cache = new Cache<T>(memoryLimitMB, expirationTime);
//   }
...
//   async loadMany(keys: K[]): Promise<(T | Error)[]> {
//     const results: (T | Error)[] = [];
//     const keysToFetch: K[] = [];
//     const keyIndexMap = new Map<K, number>();
...
//         } else if (result !== null) {
//           results[originalIndex] = result;
//           this.cache.setValue(String(key), result).catch((error) => {
...
//         } else if (result !== null) {
//           resultMap.set(keys[index], result);
//           this.cache.setValue(String(keys[index]), result).catch((error : Error) => {
//             console.error("Cache set error:", error);
//           });
//         }
//       });
...
//     await this.cache.restartAllValues();
//   }
// }
